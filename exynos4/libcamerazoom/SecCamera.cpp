...
int SecCamera::setFimcForPreview(void)
{
    ...
    void *mappedAddr = mmap(0, size, PROT_READ|PROT_WRITE, MAP_SHARED, m_exynos_mem_fd_prev, paddr);

    m_prev_mapped_addr = mappedAddr;

//    ret = setFimcDst(m_prev_fd, m_preview_width, m_preview_height, m_preview_v4lformat, (unsigned int)mappedAddr);
...
int SecCamera::runRecordFimcOneshot(int index, unsigned int src_addr)
{
    ...
    int ySize = (ALIGN(ALIGN(m_recordshot_width, 16) * ALIGN(m_recordshot_height, 16), 4096));
    int uvSize = (ALIGN(ALIGN(m_recordshot_width, 16) * ALIGN(m_recordshot_height >> 1, 8), 4096));
    int frame_size = ySize + uvSize;

    src_buf.base[0] = src_addr;
    src_buf.base[1] = src_addr + (unsigned int)(ySize);

    ret = setFimcSrc(m_rec_fd, ALIGN(m_recordshot_width, 16), ALIGN(m_recordshot_height, 16), NULL);
    CHECK(ret);

    paddr = (char *)m_buffers_record[index].phys.extP[0];

    ret = setFimcDst(m_rec_fd, m_recordshot_width, m_recordshot_height, V4L2_PIX_FMT_NV12M, (unsigned int)paddr);
    CHECK(ret);
...
unsigned int SecCamera::getShareBufferAddr(int index)
{
    ...
    return (unsigned int)fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_PADDR_Y, index);
}

int SecCamera::getSnapshotAddr(SecBuffer *buffer)
{
