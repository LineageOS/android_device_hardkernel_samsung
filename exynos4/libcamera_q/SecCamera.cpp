...
int SecCamera::startSnapshot(SecBuffer *yuv_buf)
{
    ...
    if (!m_camera_use_ISP){
        ...
        int index = fimc_v4l2_dqbuf(m_cap_fd, 1);
...
#ifndef BOARD_USE_V4L2_ION
    ret = fimc_v4l2_querybuf(m_cam_fd, m_capture_buf, V4L2_BUF_TYPE, 1, 2);
    CHECK(ret);
#endif

    ret = fimc_v4l2_qbuf(m_cam_fd, m_snapshot_width, m_snapshot_height, m_buffers_preview, 0, 1, CAPTURE_MODE);
    CHECK(ret);
...
int SecCamera::setWhiteBalance(int white_balance)
{
    ...
    if (m_params->white_balance != white_balance) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_AWB_MODE, white_balance) < 0) {
...
int SecCamera::setExposure(int exposure)
{
    ...
    if (m_camera_use_ISP) {
        ...
        if (exposure < IS_EXPOSURE_MINUS_2 || IS_EXPOSURE_PLUS_2 < exposure) {
...
int SecCamera::setImageEffect(int image_effect)
{
    ...
    if (m_params->effects != image_effect) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_IMAGE_EFFECT, image_effect) < 0) {
...
int SecCamera::setAntiBanding(int anti_banding)
{
    ...
    if (m_anti_banding != anti_banding) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_AFC_MODE, anti_banding) < 0) {
...
int SecCamera::setISO(int iso_value)
{
    ...
    if (m_params->iso != iso_value) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_ISO, iso_value) < 0) {
...
int SecCamera::setSaturation(int saturation_value)
{
    ...
    if (m_params->saturation != saturation_value) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_SATURATION, saturation_value) < 0) {
...
int SecCamera::setSharpness(int sharpness_value)
{
    ...
    if (m_params->sharpness != sharpness_value) {
        ...
        if (m_flag_camera_start) {
            if (m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_SHARPNESS, sharpness_value) < 0) {
...
int SecCamera::setMetering(int metering_value)
{
    ...
    if (m_camera_use_ISP) {
        if (metering_value < IS_METERING_CENTER || IS_METERING_MAX <= metering_value) {
...
void SecCamera::setExifChangedAttribute()
{
    ...
    //2 0th IFD Exif Private Tags
    //3 Exposure Time
    /* TODO : external isp is not shuppoting shutter speed now. */
