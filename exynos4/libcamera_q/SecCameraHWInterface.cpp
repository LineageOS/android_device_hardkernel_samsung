...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    if (cameraId == SecCamera::CAMERA_ID_BACK) {
        if (mUseInternalISP) {
            ...
            p.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES,
                  "3264x2448,1920x1080,1280x720,800x480,720x480,640x480,528x432");
        }
    } else {
		 if (mUseInternalISP) {
            //3H2
     ...
     */
    setParameters(p);
    if (cameraId == SecCamera::CAMERA_ID_BACK && mUseInternalISP)
        mSecCamera->setFrameRate(BACK_CAMERA_FPS);
    else if (mUseInternalISP)
...
int CameraHardwareSec::pictureThread()
{
    ...
    mThumbnailHeap = new MemoryHeapBaseIon(mThumbSize);
#else
    mPostviewHeap[mCapIndex] = new MemoryHeapBase(mPostViewSize);
    mThumbnailHeap = new MemoryHeapBase(mThumbSize);
#endif
#endif
    ...
    if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
        ...
        if (isLSISensor) {
            ...
            if (!SplitFrame(jpeg_data, SecCamera::getInterleaveDataSize(),
                            ...
                            mPostviewHeap[mCapIndex]->base(), &mPostViewSize)) {
                JpegHeap->release(JpegHeap);
                return UNKNOWN_ERROR;
            }
        } else {
            ...
            decodeInterleaveData(jpeg_data,
                                 ...
                                 mPostViewWidth, mPostViewHeight,
                                 &JpegImageSize, JpegHeap->data, mPostviewHeap[mCapIndex]->base());
        }
    } else
#endif
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params)
{
    ...
    const char *record_hint_str = params.get(CameraParameters::KEY_RECORDING_HINT);
    ALOGV("record_hint_str: %s", record_hint_str);

    if (record_hint_str) {
        mRecordHint = !strncmp(record_hint_str, "true", 4);
        mParameters.set(CameraParameters::KEY_RECORDING_HINT, record_hint_str);
        if (mRecordHint)
            mSecCamera->setRecording(mRecordHint);
        else
            mSecCamera->setRecording(mRecordHint);
    }
...
ALOGD(" cw:%d , ch:%d  , nw:%d ,nh:%d \n",current_preview_width,
    ...
    if ((new_AE_lock != NULL) && mPreviewRunning) {
    ...
    if (new_AWB_lock != NULL && mPreviewRunning) {
