...
int check_default_resolution(void)
{
	...
	if(nrd)	{
		if(!strncmp(buf, "1", 1))		{
		    ALOGI("%s : status == 1", DEFAULT_RESOLUTION_FILE);
			return	1;	// wakeup
		}
		else	{
			ALOGI("%s : status == 0", DEFAULT_RESOLUTION_FILE);
			return	0;	// suspend
		}
	}
...
{
    LOG_LIB_HDMI_V4L2("%s:: preset_id = 0x%x", __func__, preset_id);
...
int tvout_std_v4l2_enum_dv_presets(int fd)
{
    ...
    for (int index = 0; ; index++) {
        ...
        LOG_LIB_HDMI_V4L2("%s::index=%d, preset=0x%08x, name=%s, w=%d, h=%d",
        ...
        int rotVal, unsigned int hwc_layer)
{
    ...
    LOG_LIB_HDMI_V4L2("hdmi_set_g_scaling:: \n \\
                layer=%d,\n \\
                srcColorFormat=%d,\n \\
                src_w=%d, src_h=%d,\n\\
...
int hdmi_cable_status()
{
    ...
    LOG_LIB_HDMI_V4L2("HPD_STATUS = %d", cable_status);
...
int hdmi_check_resolution(unsigned int preset_id)
{
    ...
    if (!EDIDVideoResolutionSupport(&video)) {
        LOG_LIB_HDMI_V4L2("%s::EDIDVideoResolutionSupport(%d) fail (not suppoted preset_id) \n", __func__, preset_id);
        return -1;
    }
