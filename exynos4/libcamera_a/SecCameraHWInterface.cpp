...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    if (cameraId == SecCamera::CAMERA_ID_BACK) {
		...
		p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
                 "1280x720,800x480,720x480,640x480,320x240,528x432,176x144");
		p.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES,
                  "2048x1536,1920x1080,1280x720,800x480,720x480,640x480,528x432");
    ...
    p.set("brightness", 0);
    p.set("brightness-max", 4);
    p.set("brightness-min", -4);
...
int CameraHardwareSec::previewThread()
{
    ...
    if (mUseInternalISP && !mRecordHint) {
        ...
        }    device/hardkernel/odroidq/BoardConfig.mk \
    device/samsung/exynos4/Android.mk\
    device/samsung/exynos4/libcamerazoom_q\
    device/samsung/exynos4/libcamerazoom_q/NOTICE\
    device/samsung/exynos4/libcamerazoom_q/SecCamera.cpp\
    device/samsung/exynos4/libcamerazoom_q/SecCameraHWInterface.cpp\
    device/samsung/exynos4/libcamerazoom_q/Android.mk\
    device/samsung/exynos4/libcamerazoom_q/SecCameraHWInterface.h\
    device/samsung/exynos4/libcamerazoom_q/SecCamera.h
    }
#endif
...
//#define JPEG_DUMP
    ...
    sprintf(out_file_name, "/data/%d_%d.jpg",
            mPostViewWidth, mPostViewHeight);
    ALOGV("outFilename : %s width : %d height : %d\n",
               out_file_name, mPostViewWidth, mPostViewHeight);
    out_fp= fopen(out_file_name, "wb");

    if (out_fp == NULL) {
        ALOGE("output file open error\n");
        }

    fwrite(jpeg_data, 1,1024*1024*6, out_fp);
    fclose(out_fp);
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params)
{
	...
	//new_picture_width=2048;
	//new_picture_height=1536;
